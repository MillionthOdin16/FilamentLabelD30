...
test.describe('Core Workflow', () => {
  ...
  test('Manual Entry -> Edit -> Print', async ({ page }) => {
    ...
    await expect(page.locator('label').filter({ hasText: 'Brand' }).first()).toBeVisible();
  ...
  test('Batch Queue Addition', async ({ page }) => {
    ...
    // Verify "Added to Batch" toast
    await expect(page.getByText('Added to Batch', { exact: true })).toBeVisible();
    ...
    // Use exact match to differentiate from Toast message
    await expect(page.getByText('Batch Brand', { exact: true })).toBeVisible();
    await expect(page.getByText('PETG-BATCH', { exact: true })).toBeVisible();
  });
});
