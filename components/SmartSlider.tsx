...
const SmartSlider: React.FC<SmartSliderProps> = ({ value, onChange, min, max, safeMin, safeMax, label, unit = '' }) => {
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!isNaN(val)) {
            // Allow typing out of bounds temporarily, clamp on blur?
            // For now, immediate clamp might be annoying.
            // Let's just pass it up, but the slider position will clamp visually via getPercent logic
            onChange(val);
        }
    };
    ...
    return (
        <div className="w-full">
            <div className="flex justify-between items-center mb-2">
                ...
                <div className={`flex items-center bg-gray-800 rounded border border-gray-700 ${valueColor} focus-within:border-cyan-500 transition-colors`}>
                    <input
                        type="number"
                        value={value}
                        onChange={handleInputChange}
                        onBlur={handleBlur}
                        className="w-12 bg-transparent text-xs font-mono font-bold text-center outline-none py-0.5 appearance-none"
                    />
                    {unit && <span className="pr-2 text-[10px] opacity-70">{unit}</span>}
                </div>
            </div>
